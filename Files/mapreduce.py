...
# Implement Reverse word Index class
class ReverseIndex(MapReduce):
    
    def __init__(self, maptask, reducetask, path):
	MapReduce.__init__(self, maptask, reducetask, path)

    # Produce a (word, offset) pair for each word in value
    def Map(self, keyValue, value):
	results = []
	i = 0
	n = len(value)
	while i < n:
	    #skip no-ascii letters
	    while i < n and value[i] not in string.ascii_letters:
		i += 1
	    start = i
	    while i < n and value[i] in string.ascii_letters:
...
if __name__ == '__main__':
  ...
  out = ri.Merge()
  # Sort by word count:
  out = sorted(out, key=lambda pair: pair[0], reverse=False)
  # Print top 20:
  print "ReverseIndex:"
